...
class LennardJones:
    ...
    def get_forces(self):
        ...
        for i in range(self.N):
            for j in range(i+1,self.N):
                ...
                forces[j] -= self.force(r) # Newton's third law F_{j->i} = -F_{i->j}
        return forces
    ...
    def step(self):
        ...
        forces = self.get_forces(self.positions) #calculate the forces on all particles at time t
        self.positions[:] = self.positions + self.velocities*self.dt + 0.5*forces*self.dt**2 #update the positions
        next_forces = self.get_forces(self.positions) #calculate the forces on all particles at time t+dt
